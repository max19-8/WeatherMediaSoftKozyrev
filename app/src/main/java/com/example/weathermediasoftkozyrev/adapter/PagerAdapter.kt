...
class PagerAdapter(private val context: Context)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewPagerHolder {
        _binding = DetailViewBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: ViewPagerHolder, position: Int) {
        ...
        binding.buttonSendWeather.setOnClickListener {
            val intent = Intent().apply {
                ...
                putExtra(Intent.EXTRA_TEXT,SimpleDateFormat("dd.MM.yyyy", Locale.ROOT).format(Date(currentDay.dt*1000L)) +
                        "\n" + context.getString(R.string.weather_day,currentDay.temp.day.toString()) +
                        "\n" + context.getString(R.string.temp_max,currentDay.temp.max.toString()) +
                        "\n" + context.getString(R.string.temp_min, currentDay.temp.min.toString()))
                type = "text/plain"
            }
            holder.itemView.context.startActivity(intent)
        }
    }
   inner class ViewPagerHolder(itemBinding: DetailViewBinding) : RecyclerView.ViewHolder(itemBinding.root) {
...
//                .into(weatherIcon)
            ...
            tvSunrise.text = SimpleDateFormat("HH:mm:ss", Locale.ROOT).format(Date(day.sunrise*1000L))
            tvSunset.text = SimpleDateFormat("HH:mm:ss", Locale.ROOT).format(Date(day.sunset*1000L))
            tvPressure.text =  itemView.context.getString(R.string.pressure_value,day.pressure.toString())
            tvWindSpeed.text = itemView.context.getString(R.string.wind_speed_value,day.wind_speed.toString())
